...
public class LogDAO {
	...
	private ArrayList<LogEntry> getLogList() {
		...
		try {
			...
			Cursor c = db
					.rawQuery(
							"select _id, createdOn, content from LogEntry, LogEntryText where LogEntry._id=LogEntryText.docid order by LogEntry.createdOn",
							null);
			c.moveToFirst();
			while (!c.isAfterLast()) {
			...
			protected Void doInBackground(LogEntry... params) {
				...
				try {
					...
					SQLiteStatement stmt = db
							.compileStatement("insert into LogEntry (createdOn) values (?)");
					...
					stmt = db
							.compileStatement("insert into LogEntryText (docid, content) values (?, ?)");
		...
		AsyncTask<LogEntry, Void, Void> task = new AsyncTask<LogEntry, Void, Void>() {
			...
			protected Void doInBackground(LogEntry... params) {
				...
				try {
					...
					SQLiteStatement stmt = db
							.compileStatement("delete from LogEntry where _id=?");
					stmt.bindLong(1, e.getLogId());
					stmt.executeUpdateDelete();

					stmt = db
							.compileStatement("delete from LogEntryText where docid=?");
					stmt.bindLong(1, e.getLogId());
					stmt.executeUpdateDelete();
	...
	public void search(String searchQuery) {
		AsyncTask<String, Void, ArrayList<LogEntry>> task = new AsyncTask<String, Void, ArrayList<LogEntry>>() {
			@Override
			protected ArrayList<LogEntry> doInBackground(String... params) {
				String searchQuery = params[0] + "*";

				Logger.info("DAO searching for: %s", searchQuery);
				...
				try {
					...
					Cursor c = db.rawQuery(
						"select _id, createdOn, content from LogEntry, LogEntryText where LogEntry._id=LogEntryText.docid AND LogEntryText.content MATCH ? order by LogEntry.createdOn",
						args);
					...
					Logger.error("Failed to search log list.", e);
					receiver.onError("Failed to search log list.");
				} finally {
					if (db != null) {
						db.close();
					}
